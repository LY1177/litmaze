...
const sequelize = isProd
  ...
  : new Sequelize({
    ...
    async function init() {
      // 1) Импорт на mysql.sql (само в прод)
...
// … преди това дефиниции на модели и middleware …
...
sequelize.sync()
  ...
  .catch(err => {
    console.error('❌ Грешка при sync():', err);
    process.exit(1);
  });
...
app.get('/admin/table', async (req, res) => {
  ...
  // …тук продължава останалата логика за /admin/table
});
...
=======
  ...
  users.forEach(u => {
    ...
    `;
>>>>>>> 50f1c68f4f6b1ada9f260a97b87185142ee7aacf
  });
