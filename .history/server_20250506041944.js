...
if (process.env.NODE_ENV === 'production') {
  // за Production: postgres-backed сесии
  const PgSession = require('connect-pg-simple')(session);
  sessionStore = new PgSession({
    pool,                    // твоя pg Pool
    tableName: 'session',    // име на таблицата
    createTableIfMissing: true  // <– тук го вмъкваш
  });
} else {
  // Development: SQLite fallback
  const db = new sqlite3.Database('./mydb.db');
  pool = { /* … твоя query wrapper … */ };
  sessionStore = new session.MemoryStore();
}

// … другите ти middleware-и …
...
app.use(session({
  ...
  cookie: { maxAge: 30 * 24 * 60 * 60 * 1000 } // примерно 30 дни
}));
